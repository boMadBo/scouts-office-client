import { IStartSquad } from '@/types/leagues';
import dayjs from 'dayjs';
import { useEffect, useMemo, useState } from 'react';
import { sortPositions } from '../containers/leagues/utils';
import { config } from '@/common/config';

export const useGetSquad = (id: string | undefined) => {
  const [squad, setSquad] = useState<IStartSquad[]>([]);

  const fetchData = async () => {
    const url = `${config.transfermarkt.url}/clubs/get-squad?id=${id}&saison_id=2023&domain=com`;
    const options = {
      method: 'GET',
      headers: {
