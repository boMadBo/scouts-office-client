...
import { StartValue } from '@/interfaces/player';
import dayjs from 'dayjs';
import { useEffect, useMemo, useState } from 'react';

export const useGetValue = (id: string | undefined, key: string) => {
  const [value, setValue] = useState<StartValue[]>([]);

  const fetchData = async () => {
    const url = `${instance}/players/get-market-value?id=${id}&domain=com`;
    const options = {
      method: 'GET',
      headers: {
        'X-RapidAPI-Key': key,
