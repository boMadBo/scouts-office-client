...
import { StartTransfers } from '@/types/player';
import { useEffect, useMemo, useState } from 'react';

export const useGetTransfers = (id: string | undefined) => {
  const [transfers, setTransfers] = useState<StartTransfers[]>([]);

  const fetchData = async () => {
    const url = `${config.transfermarkt.url}/players/get-transfer-history?id=${id}&domain=com`;
    const options = {
      method: 'GET',
      headers: {
