...
import { StartStats } from '@/types/player';
import { useEffect, useMemo, useState } from 'react';

export const useGetStats = (id: string | undefined, seasonID: string = '2023') => {
  const [stats, setStats] = useState<StartStats[]>([]);

  const fetchData = async () => {
    const url = `${config.transfermarkt.url}/players/get-performance-summary?id=${id}&seasonID=${seasonID}&domain=com`;
    const options = {
      method: 'GET',
      headers: {
